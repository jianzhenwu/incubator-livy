...
object FileUtils {
  ...
  def byteCountToDisplaySize(size: BigInteger): String = {
    ...
    if (size.divide(ONE_YB).compareTo(BigInteger.ZERO) > 0) {
      getThreeSigFigs(sizeBD.divide(new BigDecimal(ONE_YB))) +
        s" YB (${String.valueOf(size)} bytes)"
    } else if (size.divide(ONE_ZB).compareTo(BigInteger.ZERO) > 0) {
      getThreeSigFigs(sizeBD.divide(new BigDecimal(ONE_ZB))) +
        s" ZB (${String.valueOf(size)} bytes)"
