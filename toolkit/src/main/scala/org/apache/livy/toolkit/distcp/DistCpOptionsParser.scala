  ...
  def parse(args: Array[String]): Config = {
    ...
    val parser = new scopt.OptionParser[Config]("") {
      ...
      opt[String]("filters")
        ...
        .text(
          "The path to a file containing a list of pattern strings, one string per line, " +
      ...
      opt[Unit]("consistentPathBehaviour")
        ...
        .text(
          "Revert the path behaviour when using overwrite or update to the path behaviour " +
            "of non-overwrite/non-update."
        )
    ...
    parser.parse(args, Config(DistCpOptions.apply())) match {
      case Some(config) =>
        config.validateUris()
        config.options.validateOptions()
        config
      case _ => throw new RuntimeException("Failed to parse arguments")
    }
  }
}

case class Config(options: DistCpOptions, URIs: Seq[URI] = Seq.empty) {
  ...
  def validateUris(): Unit = {
    require(
      URIs.length >= 2,
      "Please supply two or more paths, representing the source paths and a destination"
    )
  }
