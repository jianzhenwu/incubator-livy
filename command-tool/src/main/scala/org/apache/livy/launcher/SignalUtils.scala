...
private[launcher] object SignalUtils extends Logging {
  ...
  def register(
    ...
    try {
      val handler = handlers.getOrElseUpdate(signal, new ActionHandler(new Signal(signal)))
      handler.register(action)
    } catch {
      case ex: Exception =>
        if (logStackTrace) {
          logger.warn(failMessage, ex)
        } else {
